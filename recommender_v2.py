...
class RestaurantRecommender:
    ...
    REVIEW_CONFIDENCE_WEIGHT = 0.15  # β — how much review count matters
    RATING_DELTA_WEIGHT = 0.1        # γ — how much rating_change_vs_tripadvisor matters
    ...
    def _validate_columns(self):
        ...
        required = [
            ...
            "composite_rating", "rating_change_vs_tripadvisor",
            "nlp_composite_score",
        ] + ASPECT_COLUMNS
    ...
    def _preprocess(self):
        ...
        # rating_change_vs_tripadvisor: delta between composite_rating and
        # tripadvisor_rating (positive = NLP rates higher than TripAdvisor).
        # Normalised to [0, 1] via a sigmoid so it can be blended into scores.
        self.df["rating_change_vs_tripadvisor"] = (
            self.df["composite_rating"] - self.df["tripadvisor_rating"]
        )
        self.df["rating_change_norm"] = self.df["rating_change_vs_tripadvisor"].apply(
            lambda d: 1.0 / (1.0 + math.exp(-d))  # sigmoid centred at 0
        )
    ...
    def _compute_scores(
        ...
        γ = self.RATING_DELTA_WEIGHT   # bonus for restaurants rated higher than TripAdvisor
        aspect_w = 1.0 - α - β - γ    # remaining weight goes to aspect score

        score = (
            aspect_w * normalized_aspect
            + α * df["time_decay_norm"]
            + β * df["review_confidence"]
            + γ * df["rating_change_norm"]
        )
    ...
    def _build_result_dict(
        ...
        return {
            ...
            "tripadvisor_rating": round(float(row.get("tripadvisor_rating", 0)), 2),
            "time_decay_rating": round(float(row.get("time_decay_rating", 0)), 2),
            "rating_change_vs_tripadvisor": round(
                float(row.get("rating_change_vs_tripadvisor", 0)), 2
            ),
...
def format_recommendation(result: dict, rank: int) -> str:
    ...
    delta = result.get("rating_change_vs_tripadvisor", 0)
    delta_arrow = "▲" if delta > 0 else ("▼" if delta < 0 else "►")
    lines.append(
        f"  {delta_arrow}  vs TripAdvisor: {delta:+.2f}  "
        f"(TA: {result.get('tripadvisor_rating', 'N/A')}/5)"
    )
